...
export function About() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => setSelectedPhoto((prev) => (prev + 1) % galleryPhotos.length), 5000)
  ...
  return (
    <section id="about" className="py-12 md:py-20 lg:py-32 bg-gradient-to-br from-slate-900 via-blue-900/90 to-slate-900 relative overflow-hidden">
      <div className="absolute inset-0">
        ...
        <div className="absolute top-32 right-16 w-14 h-14 opacity-25 hover:opacity-55 transition-all duration-500 hover:scale-110 cursor-pointer group">
          <img src="/art-assets/cassiopeia.svg" alt="" className="w-full h-auto filter drop-shadow-lg group-hover:drop-shadow-blue-500/50" />
        </div>
        <div className="absolute bottom-24 left-20 w-18 h-18 opacity-35 hover:opacity-65 transition-all duration-500 hover:scale-110 cursor-pointer group">
          <img src="/art-assets/orion.svg" alt="" className="w-full h-auto filter drop-shadow-lg group-hover:drop-shadow-purple-500/50" />
        </div>
        <div className="absolute bottom-40 right-24 w-12 h-12 opacity-20 hover:opacity-50 transition-all duration-500 hover:scale-110 cursor-pointer group">
          <img src="/art-assets/pegasus.svg" alt="" className="w-full h-auto filter drop-shadow-lg group-hover:drop-shadow-yellow-500/50" />
        </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-12 sm:mb-16">
          ...
          <div className="max-w-5xl mx-auto space-y-6 sm:space-y-8 px-3 sm:px-0">
            <p className="text-base sm:text-lg md:text-xl text-gray-300 leading-relaxed">
              AWS Community Days are community-organized cloud education events that feature technical discussions and demos by expert AWS users and industry leaders from around the world. These events are designed to educate everyone, regardless of their level of AWS knowledge.
            </p>
            <div className="relative p-4 sm:p-6 lg:p-8 bg-white/5 backdrop-blur-sm rounded-2xl border border-orange-500/20 shadow-2xl">
                  ...
                  <div className="bg-gradient-to-br from-orange-400 via-orange-500 to-amber-600 p-0.5 rounded-lg shadow-2xl">
                    <div className="bg-gradient-to-br from-orange-500 via-amber-500 to-orange-600 rounded-lg px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2 relative overflow-hidden">
                      ...
                      <div className="relative flex items-center justify-center text-orange-900">
                        ...
                        <span className="font-black text-[10px] sm:text-xs md:text-sm tracking-wide drop-shadow-sm whitespace-nowrap">JOIN THE MOVEMENT</span>
                      </div>
              ...
              <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-white leading-relaxed font-semibold pt-6 sm:pt-4 md:pt-2">
                Join <span className="text-orange-400 font-black">200+</span> developers, architects, and cloud enthusiasts in Cebu for an unforgettable day of learning, networking, and innovation. Experience hands-on workshops, inspiring keynotes, and connect with the vibrant AWS community in the Philippines.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 sm:mt-16 md:mt-20">
          ...
          <div className={`max-w-6xl mx-auto transition-all duration-1000 ${isVisible ? "animate-fade-in" : "opacity-0"}`}>
            <div className="relative mb-4 sm:mb-6 md:mb-8 group">
              <div className="relative rounded-lg sm:rounded-xl md:rounded-2xl overflow-hidden shadow-2xl" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
                <div className="aspect-[4/3] sm:aspect-[3/2] md:aspect-[16/9] relative">
                  <OptimizedImage src={galleryPhotos[selectedPhoto].src} alt={galleryPhotos[selectedPhoto].alt} fill className={`object-cover transition-all duration-700 ease-out ${isImageLoading ? "opacity-70 scale-105" : "opacity-100 scale-100"}`} sizes="(max-width: 768px) 100vw, 1200px" priority loading="eager" />
                  {isImageLoading && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/20 backdrop-blur-sm">
                      <div className="w-8 h-8 border-2 border-orange-500 border-t-transparent rounded-full animate-spin"></div>
                    </div>
                  )}
                  <button onClick={goToPrevious} className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-2 sm:p-3 rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110 touch-manipulation min-w-[44px] min-h-[44px] flex items-center justify-center" aria-label="Previous image">
                    <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />
                  </button>
                  <button onClick={goToNext} className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white p-2 sm:p-3 rounded-full transition-all duration-300 opacity-0 group-hover:opacity-100 hover:scale-110 touch-manipulation min-w-[44px] min-h-[44px] flex items-center justify-center" aria-label="Next image">
            ...
            <div className="flex justify-center mb-4 sm:mb-6 md:mb-8">
              <div className="flex gap-1 sm:gap-2 p-1 sm:p-2 bg-white/5 backdrop-blur-sm rounded-lg sm:rounded-xl md:rounded-2xl border border-white/10 overflow-x-auto max-w-full scrollbar-hide">
                {galleryPhotos.map((photo, index) => (
                  <button key={photo.id} onClick={() => goToSlide(index)} className={`relative w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 lg:w-20 lg:h-20 rounded-md sm:rounded-lg md:rounded-xl overflow-hidden transition-all duration-300 flex-shrink-0 touch-manipulation ${selectedPhoto === index ? "ring-2 ring-orange-500 scale-110 shadow-lg shadow-orange-500/25" : "hover:scale-105 opacity-70 hover:opacity-100"}`} aria-label={`View ${photo.title}`}>
                    <OptimizedImage src={photo.src} alt={photo.alt} fill className="object-cover" sizes="(max-width: 640px) 40px, (max-width: 768px) 48px, (max-width: 1024px) 64px, 80px" />
                    {selectedPhoto === index && (
