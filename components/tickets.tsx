...
import { Clock, CheckCircle, Star, ExternalLink, Sparkles, X } from "lucide-react"
import { ticketTypes, EARLY_BIRD_END_DATE, TICKETS_URL } from "@/data"
...
export function Tickets() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const distance = EARLY_BIRD_END_DATE.getTime() - now
      if (distance > 0) {
        setTimeLeft({
  ...
  const handleRegisterClick = () => {
    window.open(TICKETS_URL, "_blank", "noopener,noreferrer")
  }
    ...
    <section id="tickets" className="py-12 md:py-20 lg:py-32 bg-gradient-to-br from-slate-900 via-blue-900/90 to-slate-900 relative overflow-hidden">
      <div className="absolute inset-0 constellation-background">
        <div className="constellation-container">
          <div className="constellation-svg animate-ray-pulse" style={{ top: "60%", left: "15%", width: "200px", height: "auto", opacity: "0.04", "--ray-rotation": "25deg" } as React.CSSProperties}>
            <img src="/art-assets/ray.svg" alt="" className="w-full h-auto" />
          </div>
          <div className="constellation-svg animate-ray-pulse" style={{ bottom: "30%", right: "18%", width: "180px", height: "auto", opacity: "0.06", "--ray-rotation": "-40deg" } as React.CSSProperties}>
            <img src="/art-assets/ray.svg" alt="" className="w-full h-auto" />
          </div>
          <div className="constellation-svg animate-shroud-depth" style={{ top: "20%", left: "10%", width: "300px", height: "auto", opacity: "0.03" }}>
            <img src="/art-assets/shroud.svg" alt="" className="w-full h-auto" />
          </div>
          <div className="constellation-svg constellation-hover-spin" style={{ top: "35%", left: "8%", width: "70px", height: "auto" }}>
            <img src="/art-assets/cassiopeia.svg" alt="" className="w-full h-auto constellation-glow-orange animate-gentle-shimmer" style={{ opacity: "0.4" }} />
          </div>
          <div className="constellation-svg constellation-hover-spin" style={{ bottom: "25%", right: "12%", width: "65px", height: "auto" }}>
            <img src="/art-assets/pegasus.svg" alt="" className="w-full h-auto constellation-glow-orange animate-gentle-shimmer-delayed" style={{ opacity: "0.35" }} />
          </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-8 sm:mb-12 md:mb-16 lg:mb-20">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl 2xl:text-7xl font-black text-white mb-4 sm:mb-6 md:mb-8 tracking-tight leading-tight px-2 sm:px-0">
            ...
            <span className="bg-gradient-to-r from-orange-400 via-yellow-500 to-orange-600 bg-clip-text text-transparent block sm:inline">Spot Today</span>
          </h2>
          <p className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-300 max-w-4xl mx-auto leading-relaxed px-3 sm:px-4 md:px-0">
            Join 200+ developers, architects, and cloud enthusiasts for an unforgettable day of learning and networking. Choose the ticket that works best for you.
          </p>
        </div>

        {isEarlyBirdActive && <EarlyBirdCountdown timeLeft={timeLeft} />}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-7xl mx-auto mb-8 sm:mb-12 md:mb-16 lg:mb-20">
          {ticketTypes.map((ticket, index) => (
            <TicketCard key={index} ticket={ticket} index={index} isVisible={isVisible} isEarlyBirdActive={isEarlyBirdActive} onRegister={handleRegisterClick} />
          ))}
        </div>
        ...
        <div className="text-center">
          <p className="text-gray-400 text-lg">
            Powered by <span className="font-bold text-orange-400">TicketNation</span> - All prices are in Philippine Pesos - Tickets are non-refundable but transferable
          </p>
        </div>
      </div>
    </section>
...
function EarlyBirdCountdown({ timeLeft }: { timeLeft: { days: number; hours: number; minutes: number; seconds: number } }) {
  const items = [
          ...
          <div className="text-center">
            <div className="inline-flex flex-col sm:flex-row items-center gap-2 bg-gradient-to-r from-orange-500 to-yellow-500 text-white rounded-2xl px-4 py-3 shadow-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold">Save up to P200</span>
              </div>
              <div className="text-xs opacity-90 text-center">Early bird until Aug 30, 2025</div>
            </div>
          </div>
        </div>
      </div>
...
function TicketCard({ ticket, index, isVisible, isEarlyBirdActive, onRegister }: TicketCardProps) {
  const Icon = ticket.icon

  return (
    <div className={`transition-all duration-1000 flex ${isVisible ? "animate-slide-up" : "opacity-0 translate-y-10"} ${ticket.popular ? "sm:col-span-2 lg:col-span-1" : ""}`} style={{ animationDelay: `${index * 200}ms` }}>
      <Card className={`relative border-2 transition-all duration-700 hover:scale-105 w-full flex flex-col group ${ticket.soldOut ? "border-gray-500/30 bg-gray-900/50 opacity-75" : ticket.popular ? "border-orange-500/50 shadow-2xl shadow-orange-500/30 bg-gradient-to-br from-orange-500/10 via-yellow-500/5 to-orange-500/10" : "border-white/20 hover:border-orange-500/30 hover:shadow-2xl hover:shadow-orange-500/20 bg-white/5"} backdrop-blur-sm`}>
        <div className={`absolute inset-0 bg-gradient-to-r ${ticket.soldOut ? "from-gray-500/10 to-gray-600/10" : ticket.glowColor === "orange" ? "from-orange-500/20 to-yellow-500/20" : ticket.glowColor === "gray" ? "from-gray-500/20 to-gray-600/20" : "from-purple-500/20 to-pink-500/20"} rounded-2xl blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 -z-10`}></div>
        ...
        <CardHeader className="text-center pb-2 sm:pb-3 md:pb-4 lg:pb-6 relative p-3 sm:p-4 md:p-5 lg:p-6">
          ...
          <div className="mb-2 sm:mb-3 md:mb-4 lg:mb-6">
            <div className={`w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 lg:w-20 lg:h-20 mx-auto mb-2 sm:mb-3 md:mb-4 lg:mb-6 rounded-full ${ticket.badgeColor} flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-500`}>
              <Icon className="w-5 h-5 sm:w-6 sm:h-6 md:w-8 md:h-8 lg:w-10 lg:h-10 text-white" />
            </div>
            {!ticket.popular && !ticket.soldOut && (
          ...
          <div className="flex items-center justify-center gap-2 sm:gap-3 mb-2 sm:mb-3 lg:mb-4">
            {ticket.soldOut ? (
              ...
              <>
                <span className="text-xl sm:text-2xl md:text-3xl lg:text-5xl font-black bg-gradient-to-r from-orange-400 to-yellow-500 bg-clip-text text-transparent">{ticket.earlyBirdPrice}</span>
                <span className="text-sm sm:text-base md:text-lg lg:text-xl text-gray-400 line-through">{ticket.price}</span>
              </>
            ) : (
              <span className="text-xl sm:text-2xl md:text-3xl lg:text-5xl font-black bg-gradient-to-r from-orange-400 to-yellow-500 bg-clip-text text-transparent">{ticket.price}</span>
            )}
          </div>
          <p className="text-xs sm:text-sm md:text-base text-gray-300 leading-relaxed group-hover:text-white transition-colors duration-500">{ticket.description}</p>
        </CardHeader>
          ...
          <ul className="space-y-2 sm:space-y-3 lg:space-y-4 flex-1 mb-3 sm:mb-4 md:mb-6 lg:mb-8">
            {ticket.features.map((feature, featureIndex) => (
              <li key={featureIndex} className="flex items-start gap-2 sm:gap-3 group/item">
                ...
                <span className="text-xs sm:text-sm md:text-base text-gray-300 leading-relaxed group-hover:text-white transition-colors duration-300">{feature}</span>
              </li>
            ))}
          </ul>
          <div className="mt-auto pt-2 sm:pt-3 md:pt-4">
            <Button onClick={ticket.soldOut ? undefined : onRegister} disabled={ticket.soldOut} className={`w-full py-3 sm:py-4 font-bold text-sm sm:text-base transition-all duration-500 transform min-h-[44px] touch-manipulation ${ticket.soldOut ? "bg-gray-600/50 text-gray-400 border border-gray-600/30 cursor-not-allowed opacity-60" : ticket.popular ? "bg-gradient-to-r from-orange-500 to-yellow-500 text-white hover:from-orange-600 hover:to-yellow-600 shadow-xl shadow-orange-500/30 hover:shadow-orange-500/50 hover:scale-105 active:scale-95" : "bg-white/5 text-orange-400 border border-orange-500/30 hover:bg-orange-500/10 hover:border-orange-500/50 backdrop-blur-sm hover:text-white hover:scale-105 active:scale-95"} group/button`}>
              {ticket.soldOut ? (
                <>
